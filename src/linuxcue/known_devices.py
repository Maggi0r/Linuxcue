...
TARGET_DEVICES: tuple[KnownDevice, ...] = (
    ...
    KnownDevice(
        ...
        protocol="wireless-audio+hid",
        support_level="audio-mapped",
        next_step="Use the safe Linux PipeWire EQ path now. Capture Windows iCUE before/after HID writes before enabling dongle RGB, battery, pairing, or headset controls.",
        companion_slug=None,
        companion_role="wireless transport for VOID Elite headset mode",
        capabilities=("device-detection", "audio-standard", "eq-15-band-pipewire", "wireless-dongle-detected", "hid-live-write-blocked"),
        ...
        mock_notes=(
            ...
            "PipeWire EQ is safe because it does not write HID commands to the dongle.",
            "Dongle HID writes stay disabled until VOID Elite command captures are available.",
        ),
    ),
)
